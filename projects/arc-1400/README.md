...
By default the template creates a single `HelloWorld` contract under security_token folder in the `smart_contracts` directory. To add a new contract:
...
2. Decide what values you want to use for the `allowUpdate` and `allowDelete` parameters specified in [`deploy-config.ts`](./smart_contracts/security_token/deploy-config.ts).
