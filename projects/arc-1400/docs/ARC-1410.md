...
Portion operators provide granular delegation without granting full operator rights. If both a full operator authorization and a portion allowance exist, full authorization takes precedence and does not consume the allowance. Portion allowance is decremented on each successful `arc1410_operator_transfer_by_partition` that relies on it.

#### 3.5.1 arc1410_can_transfer_by_partition
...
`arc1410_operator_transfer_by_partition` MUST succeed if either:
...
If path (2) is used, the allowance MUST be decremented atomically by the transferred amount. If both apply, do not decrement the allowance.
