...
- Implementations MAY forbid bare ARC-200 transfers entirely (forcing `transfer_by_partition`) by having the ARC-200 logic route through the application; this MUST be clearly documented if chosen.
...
- Partition-aware issuance/redemption (`issue_by_partition`, `redeem_by_partition`) directly adjust both the ARC-200 total and the specified partition balance.
...
- `partitions_of(holder: address, page: uint64) -> (partitions: address[])` (MAY paginate if large. if not, page = 0)
